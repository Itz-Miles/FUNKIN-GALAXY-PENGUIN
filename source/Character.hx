...
class Character extends FlxSprite
{
	...
	public function act(arguments:Array<Dynamic>)
	{
		...
		switch (theAction)
		{
			...
			case 'equip':
				// play sound item pop
				this.heldItem = theArgumentsArray[0];
			// update the offhand
			...
			case 'fade':
				...
				} // might give multiple characters their own individual icons
			...
			case 'attack':
				...
				switch (theArgumentsArray[4])
				{
					...
					case 'fire':
					// fires a rocket, explosion might kill, more effective if the opponent is stunned
					case 'stun':
			...
			case 'jump':
				...
				if (overrideGravity != 'false')
					this.acceleration.y = this.overrideGravity;
			// collision in update

			// reset variables

			case 'move':
				addVelocityX = theArgumentsArray[1];
				this.velocity.x += addVelocityX;
				if (theArgumentsArray[1] != 'false')
				{
					overrideDragX = theArgumentsArray[0];
				...
				if (this.isPlayer)
				{
					FlxTween.tween(FlxG.camera, {zoom: 1.5}, 1, {
						...
						onComplete: function(twn:FlxTween)
						{
							...
							FlxTween.color(this, 0.5, FlxColor.WHITE, 0xFFe6726a, {
			...
			case 'respawn':
				...
				FlxTween.tween(FlxG.camera, {zoom: 1}, 1, {
					...
					onComplete: function(twn:FlxTween)
					{
						...
						FlxTween.color(this, 0.5, 0xFFe6726a, FlxColor.WHITE, {
							ease: FlxEase.quadInOut,
							onComplete: function(twn:FlxTween)
							{
								new FlxTimer().start(1, function(tmr:FlxTimer)
